...
/datum/controller/subsystem/ticker/proc/PostSetup()
	...
	send2adminchat("Server", "Round [GLOB.round_id ? "#[GLOB.round_id]:" : "of"] has started[allmins.len ? ".":" with no active admins online!"]")
	setup_done = TRUE
...
/datum/controller/subsystem/ticker/proc/get_captain_or_backup()
	...
	for(var/mob/dead/new_player/new_player_mob as anything in GLOB.new_player_list)
		if(is_banned_from(new_player_mob.ckey, list(JOB_PORT_AUTHORITY)))
			CHECK_TICK
			continue
